...
export default function DeviceSlider(config: SliderConfig) {
  ...
  const handleRelease = async (event: any, newValue: number | number[]) => {
    setValue(newValue as number);
    const request = { "id": "8", "jsonrpc": "2.0", "method": "Client.SetVolume", "params": { "id": config.id, "volume": { "muted": false, "percent": newValue } } };
  ...
  useEffect(() => {
    ...
    onVolumeChanged()
  }, [groupMuted, deviceMuted, config.groupId, config.id])
  ...
  return (
    <div className={classes.root}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs>
          <Slider disabled={groupMuted || deviceMuted} value={value} onChange={handleDrag} onChangeCommitted={handleRelease} aria-labelledby="continuous-slider" />
        </Grid>
        <Grid item>
          <VolumeUp />
        </Grid>
